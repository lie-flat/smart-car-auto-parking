...
from ..config.rl import REAL2SIM, SIM_SCALE
...
# fmt: off
def update_rl_info(sim_tr, velocity, z_rotation, z_cos_t, z_sin_t):
    ...
    draw_rl_info(text=f" Success  : {analytics['success']}", org=(10, 175))
    draw_rl_info_with_color(text=f"    > Observation Data <", org=(10, 210), color=(255,255,0))
    draw_rl_info(text=f"  X Pos   : {SIM_SCALE * sim_tr[0,3].item():.8f}", org=(10, 245))
    draw_rl_info(text=f"  Y Pos   : {SIM_SCALE * sim_tr[1,3].item():.8f}", org=(10, 280))
...
def main():
    ...
    global img_result, rl_info_area, last_pos
    ...
    while True:
        ...
        if translation_world is not None:
            ...
            velocity = SIM_SCALE * (sim_tr[:2, 3] - last_pos) / delta_t
        ...
        update_rl_info(sim_tr, velocity, z_rotation, z_cos_t, z_sin_t)
        new_frame_time = time()
